...
export function BetControls({
  ...
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4">
      <button
        aria-label="Bet that the next hand is higher"
        disabled={disabled}
      ...
      >
        <span aria-hidden className="text-3xl leading-none transition-transform duration-200 group-hover:-translate-y-0.5">
          ▲
        </span>
      ...
      <button
        aria-label="Bet that the next hand is lower"
        disabled={disabled}
        onClick={() => onBet('lower')}
